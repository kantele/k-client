...
App.prototype.writeScripts = function(store, dir, options, cb) {
  ...
  this.bundle(store, options, function(err, source, map) {
    ...
    app.scriptUrl = (options && options.SCRIPTROOT || '') + '/k-client/' + filename + '.js';

    // Write current map and bundle files
    if (!(options && options.disableScriptMap)) {
      app.scriptMapUrl = '/k-client/' + filename + '.map.json';
    ...
    if (options && options.s3) {
      options.s3.putFile(app.scriptFilename, '/k-client/' + filename + '.js', { 'x-amz-acl': 'public-read' }, function (err, res) {
        console.log('s3.putFile err', err);
        console.log('s3.putFile res', res);
      });
    }
