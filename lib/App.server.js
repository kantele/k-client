...
App.prototype._init = function() {
  ...
  this.views.register('Page',
    ...
    '<view name="{{$render.prefix}}Favicon"></view>' +
...
App.prototype.bundle = function(store, options, cb) {
  ...
  store.once('bundle', function(bundle) {
    bundle.require(path.dirname(__dirname), {expose: 'k-client'});
...
App.prototype.writeScripts = function(store, dir, options, cb) {
  ...
  this.bundle(store, options, function(err, source, map) {
    if (err) return cb(err);
    dir = path.join(dir, 'k-client');
    if (!fs.existsSync(dir)) fs.mkdirSync(dir);

    var filename = app.name + '-' + app.scriptHash,
        base = path.join(dir, filename),
        scriptUrl = (options.upload && options.upload.folder || '') + '/k-client/' + filename + '.js';

    app.scriptUrl = (options.upload && options.upload.url || '') + scriptUrl;

    // Write current map and bundle files
    if (!options.disableScriptMap) {
      app.scriptMapUrl = (options.scriptMapHost? options.scriptMapHost.replace(/\/$/, '') : '') + '/k-client/' + filename + '.map.json';
    ...
    if (options.upload && options.upload.s3) {
      console.log('uploading to ' + scriptUrl);
      options.upload.s3.putFile(app.scriptFilename, scriptUrl, { 'x-amz-acl': 'public-read' }, function (err, res) {
        console.log('upload done. ', res && res.statusCode);
        if (err) {
          console.error('S3.putFile err', err);
        }
      });
    }
...
App.prototype._autoRefresh = function(store) {
  ...
  store.on('client', function(client) {
    client.on('close', function() {
      ...
      app.emit('client-remove', client.id);
    });
    client.channel.on('derby:app', function(data, cb) {
      ...
      app.emit('client-add', client);
      cb();
    });
  });
};

App.prototype.addClientRegistrations = function(store) {
  var clients = this.clients = {};
  var app = this;
  store.on('client', function(client) {
    client.on('close', function() {
